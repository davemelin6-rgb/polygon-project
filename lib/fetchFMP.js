...
export async function fetchFundamentals(ticker, apiKey) {
  ...
  // Use quarterly statements for fresher data (last ~3 months vs last ~15 months for annual)
  const [incomeCurrent, incomePrior, balance, cashflow] = await Promise.all([
    ...
    fmpGet(`/balance-sheet-statement?symbol=${ticker}&period=quarter&limit=1`, apiKey, 0),
    fmpGet(`/cash-flow-statement?symbol=${ticker}&period=quarter&limit=1`, apiKey, 0),
  ]);
