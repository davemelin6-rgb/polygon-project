...
export async function fetchFundamentals(ticker, apiKey) {
  ...
  const [
    ...
    fmpGet(`/key-metrics?symbol=${ticker}&limit=1`, apiKey, 0),
    // Earnings surprises — last 4 quarters of beat/miss
    fmpGetAll(`/earnings?symbol=${ticker}&limit=4`, apiKey, 4),
  ...
  if (Array.isArray(earningsSurprises) && earningsSurprises.length > 0) {
    const surprises = earningsSurprises
      ...
      .map(e => e.epsEstimated !== 0
        ? (e.epsActual - e.epsEstimated) / Math.abs(e.epsEstimated)
        : 0
      )
      .filter(v => v !== null);
  ...
  return {
    // Data freshness — how old is the most recent filing?
    filingDate: incomeCurrent.fillingDate || incomeCurrent.date || null,
    filingPeriod: incomeCurrent.period || null, // "annual" or "Q1/Q2/Q3/Q4"
    dataAgeDays: (() => {
      const d = incomeCurrent.fillingDate || incomeCurrent.date;
      if (!d) return null;
