...
export function calcMomentum({ price, aggs, fundamentals, vix }) {
  ...
  // ── Momentum Acceleration (NEW) ─────────────────────────────
  // Is momentum SPEEDING UP or slowing down?
  // recent3M vs prior3M: positive = accelerating (strong buy signal)
  // negative = decelerating (thesis weakening)
  // Fully historical — price-based only, no look-ahead.
  const prior3M        = p130 > 0 ? (p60 - p130) / p130 : 0; // 3M–6M return
  const acceleration   = ret3M - prior3M;                      // delta
  const accelScore     = normalize(acceleration, -0.25, 0.25);
  ...
  // Weighted score — acceleration is the new primary differentiator
  let score = (
    ret6MScore    * 0.30 +
    accelScore    * 0.25 +  // momentum acceleration — new
    ret3MScore    * 0.20 +
    maScore       * 0.18 +
    surpriseScore * 0.05 +
    relVolScore   * 0.02
...
export function calcRisk({ aggs, fundamentals }) {
  ...
  return Math.round(clamp(100 - rawRisk, 0, 100));
}

// ── TECH QUALITY (0-100) ────────────────────────────────────────
// Renamed from TECH VALUE. Measures TODAY's business quality:
// margins, FCF, ROE, R&D intensity, earnings consistency.
// "How good is the existing business right now?"
// Sector-normalised — semiconductor at 50% gross margin ≠ weak.
export function calcTechQuality({ fundamentals, benchmarks }) {
...
export const calcTechValue = calcTechQuality;

// ── TECH DEMAND (0-100) ──────────────────────────────────────────
// "How much will the WORLD NEED this technology in 1-5 years?"
// Combines editorial sector demand score + analyst consensus + revenue trajectory.
// Works for pre-profit companies — IONQ can score high even with no earnings
// if the sector demand trajectory is strong and analysts are bullish.
export function calcTechDemand({ fundamentals, sectorDemandScore, sectorEtfAggs }) {

  // 1. Sector demand score (30%) — editorial assessment, updated quarterly
  // Reflects: technology adoption stage, market size growth, government/enterprise demand
  const demandScore = sectorDemandScore ?? 70;

  // 2. Analyst forward revenue growth (30%) — what Wall Street consensus expects next year
  // High forward growth = market believes demand is strong
  const analystGrowth = fundamentals?.analystRevenueGrowth ?? null;
  const analystScore  = analystGrowth != null
    ? normalize(analystGrowth, -0.05, 0.60)
    : 50;

  // 3. Sector ETF momentum (25%) — how is the sector performing vs the broader market?
  // A rising sector ETF = institutional money flowing into this technology
  // Uses 3-month return of the sector ETF as proxy for institutional conviction
  let etfScore = 50;
  if (sectorEtfAggs && sectorEtfAggs.length >= 60) {
    const now  = sectorEtfAggs.at(-1).c;
    const p60  = sectorEtfAggs[sectorEtfAggs.length - 60].c;
    const etfRet3M = (now - p60) / p60;
    etfScore = normalize(etfRet3M, -0.15, 0.35);
  }

  // 4. Revenue growth trajectory (15%) — is the company's revenue accelerating?
  // Acceleration = market demand is building, not plateauing
  const currentGrowth = fundamentals?.income?.revenueGrowth ?? 0;
  const analystFwd    = fundamentals?.analystRevenueGrowth ?? currentGrowth;
  const trajectory    = analystFwd - currentGrowth; // positive = demand accelerating
  const trajectoryScore = normalize(trajectory, -0.20, 0.30);
...
// ── SENTIMENT (0-100) ────────────────────────────────────────────
// "Are experts and the market getting MORE bullish or less?"
// Measures the DIRECTION of expert opinion, not just the level.
// A stock where analysts are rapidly upgrading estimates is fundamentally
// different from one where estimates are flat or being cut.
export function calcSentiment({ fundamentals }) {
  if (!fundamentals) return null;

  // 1. Analyst estimate revision direction (40%)
  // Compare analyst forward revenue estimate vs current annual growth
  // If analysts expect MORE growth than the stock has shown → bullish revision
  const currentGrowth  = fundamentals.income?.revenueGrowth ?? 0;
  const analystFwd     = fundamentals.analystRevenueGrowth ?? null;
  const revisionDelta  = analystFwd != null ? analystFwd - currentGrowth : null;
  const revisionScore  = revisionDelta != null
    ? normalize(revisionDelta, -0.20, 0.30)
    : 50;

  // 2. Earnings beat consistency (35%)
  // Companies that consistently beat analyst expectations have positive sentiment momentum
  // The market re-rates them upward with each beat
  const surpriseAvg    = fundamentals.earningsSurpriseAvg ?? null;
  const surpriseScore  = surpriseAvg != null
    ? normalize(surpriseAvg, -0.15, 0.20)
    : 50;

  // 3. Analyst revenue growth conviction (25%)
  // How bullish is the absolute analyst consensus?
  // High absolute forward growth = strong positive sentiment
  const analystScore = analystFwd != null
    ? normalize(analystFwd, -0.05, 0.50)
    : 50;
...
// Returns the raw momentum acceleration value (not 0-100 scaled).
// recent3M − prior3M: positive = building, negative = fading.
// Used to detect Grade B → Grade A transition setups.
export function calcAcceleration({ aggs }) {
  if (!aggs || aggs.length < 130) return null;
  const now  = aggs.at(-1).c;
  const p60  = aggs[aggs.length - 60].c;
  const p130 = aggs[aggs.length - 130].c;
  if (!p60 || !p130) return null;
  const recent3M = (now - p60)  / p60;
  const prior3M  = (p60 - p130) / p130;
  return Math.round((recent3M - prior3M) * 1000) / 1000; // 3 decimal precision
}

// ── INNOVATION (0-100) ───────────────────────────────────────────
// Measures technology investment quality and growth trajectory.
// Designed for pre-profit companies — IONQ, MRNA, RGTI can score high
// even with no earnings, as long as R&D is converting and revenue is accelerating.
export function calcInnovation({ fundamentals, benchmarks }) {
  if (!fundamentals) return null;

  const { income, analystRevenueGrowth, grossMarginPrior, rdSpendPrior } = fundamentals;
  const rd  = income.researchAndDevelopmentExpenses || 0;
  const rev = income.revenue || 0;

  // 1. R&D Intensity (25%) — R&D / Revenue, sector-normalised
  // Tells us: is the company seriously investing in technology?
  const rdIntensity      = rev > 0 ? rd / rev : null;
  const rdIntensityScore = rdIntensity != null
    ? normalize(rdIntensity, benchmarks.rdIntensity[0], benchmarks.rdIntensity[1])
    : 50;

  // 2. R&D Productivity (25%) — revenue growth dollars / R&D spend
  // Tells us: is the R&D actually converting into business results?
  const revGrowthDollars  = rev * (income.revenueGrowth ?? 0);
  const rdProductivity    = rd > 0 ? revGrowthDollars / rd : null;
  const rdProductivityScore = rdProductivity != null
    ? normalize(rdProductivity, -1, 3)
    : 50;

  // 3. Revenue Acceleration (25%) — forward analyst growth vs current growth
  // Tells us: is adoption accelerating? Are experts expecting more?
  const currentGrowth    = income.revenueGrowth ?? 0;
  const forwardGrowth    = analystRevenueGrowth ?? currentGrowth;
  const acceleration     = forwardGrowth - currentGrowth;
  const accelerationScore = normalize(acceleration, -0.20, 0.30);

  // 4. Gross Margin Trajectory (15%) — is pricing power improving?
  // Even pre-profit companies should have improving gross margins over time.
  const currentGM    = rev > 0 ? income.grossProfit / rev : null;
  const trajectory   = grossMarginPrior != null && currentGM != null
    ? currentGM - grossMarginPrior
    : null;
  const trajectoryScore = trajectory != null
    ? normalize(trajectory, -0.10, 0.10)
    : 50;

  // 5. Analyst Conviction (10%) — how bullish is consensus on future revenue?
  const analystScore = analystRevenueGrowth != null
    ? normalize(analystRevenueGrowth, -0.10, 0.60)
    : 50;

  const score = Math.round(
    rdIntensityScore    * 0.25 +
    rdProductivityScore * 0.25 +
...
export function calcSignal({ momentum, risk, techValue = null, innovation = null, techDemand = null, sentiment = null }) {
  if (momentum === null) return null;
  const r = risk ?? 50;

  // Count available fundamental scores
  const hasDemand    = techDemand  !== null;
  const hasSentiment = sentiment   !== null;
  const hasInnov     = innovation  !== null;
  const hasTechQ     = techValue   !== null;

  if (hasDemand && hasSentiment && hasInnov && hasTechQ) {
    // Full 6-score model: MOM 35% + DEMAND 20% + SENTIMENT 15% + INNOV 15% + TECH 10% + RISK 5%
    return Math.round(
      momentum   * 0.35 +
      techDemand * 0.20 +
      sentiment  * 0.15 +
  ...
  if (hasDemand && hasSentiment) {
    // Demand + sentiment but no INNOV/TECH: MOM 40% + DEMAND 25% + SENTIMENT 20% + RISK 15%
    return Math.round(momentum * 0.40 + techDemand * 0.25 + sentiment * 0.20 + r * 0.15);
  }
  if (hasInnov && hasTechQ) {
    // Legacy full model: MOM 45% + INNOV 30% + TECH 15% + RISK 10%
    return Math.round(momentum * 0.45 + innovation * 0.30 + techValue * 0.15 + r * 0.10);
  }
  if (hasInnov) {
    return Math.round(momentum * 0.50 + innovation * 0.35 + r * 0.15);
  }
  if (hasTechQ) {
    return Math.round(momentum * 0.50 + techValue * 0.35 + r * 0.15);
  }
  return Math.round(momentum * 0.75 + r * 0.25);
}
