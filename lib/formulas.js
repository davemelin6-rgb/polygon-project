...
export function calcMomentum({ price, aggs, fundamentals, vix }) {
  ...
  const ret3MScore = normalize(ret3M, -0.40, 0.80);

  // 6-month return (130 trading days) — primary driver
  // Jegadeesh & Titman (1993): momentum works on 3-12 month formation, not 1-3 months
  // 1-month removed — too short, captures mean reversion not continuation
  const ret6M      = (now - p130) / p130;
  const ret6MScore = normalize(ret6M, -0.50, 1.20);
