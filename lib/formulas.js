...
export function calcSignal({ momentum, risk, techValue = null, innovation = null }) {
  if (momentum === null) return null;
  const r = risk ?? 50;

  if (innovation !== null && techValue !== null) {
    // Full signal: MOM 45% + INNOV 30% + TECH 15% + RISK 10%
    return Math.round(momentum * 0.45 + innovation * 0.30 + techValue * 0.15 + r * 0.10);
  }
  if (innovation !== null) {
    // No tech value: MOM 50% + INNOV 35% + RISK 15%
    return Math.round(momentum * 0.50 + innovation * 0.35 + r * 0.15);
  }
  if (techValue !== null) {
    // No innovation: MOM 50% + TECH 35% + RISK 15%
