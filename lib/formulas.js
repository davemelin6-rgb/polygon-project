...
export function calcMomentum({ price, aggs, fundamentals }) {
  ...
  // Earnings surprise: consistent beaters have fundamental backing for momentum
  // Avg surprise of last 4 quarters — positive = beats, negative = misses
  const surpriseAvg = fundamentals?.earningsSurpriseAvg ?? null;
  const surpriseScore = surpriseAvg !== null
    ? normalize(surpriseAvg, -0.20, 0.20) // -20% miss to +20% beat
  ...
  // If the stock is down hard AND trading below both MAs, we never
  // assign a high score — buying into a confirmed downtrend is a death march.
  const belowBothMAs = now < ma50 && now < ma200;
  const sharpDecline = ret1M < -0.15; // down more than 15% in a month
  if (belowBothMAs && sharpDecline) {
    score = score * 0.50; // cut score in half — strong downtrend signal
  }
...
// Consistent earnings beaters with high ROE and expanding margins = strong moat.
export function calcTechValue({ fundamentals }) {
  if (!fundamentals) return null;

  const { income, cashflow, metrics, earningsSurpriseAvg } = fundamentals;
  ...
  // Net margin: full cost structure — captures operational efficiency gross margin misses
  const netMargin  = (income.netIncome || 0) / revenue;
  const nmScore    = normalize(netMargin, -0.10, 0.35);

  // Revenue growth YoY (annual preferred, QoQ fallback)
  const revGrowth   = income.revenueGrowth || 0;
  const growthScore = normalize(revGrowth, -0.10, 0.50);

  // FCF margin: capital efficiency
  const fcfMargin = (cashflow.freeCashFlow || 0) / revenue;
  const fcfScore  = normalize(fcfMargin, -0.10, 0.30);

  // ROE: how efficiently management uses shareholder capital
  const roe = metrics?.roe ?? null;
  const roeScore = roe !== null
    ? normalize(roe, -0.10, 0.40) // negative ROE = bad, 40%+ ROE = exceptional
    : 50;

  // Earnings surprise consistency: consistent beaters have durable competitive advantage
