...
/* ─── value props shown on left panel ───────────────────── */
const PROPS = [
  {
    icon: "☀️",
  ...
  },
  {
    ...
    text: "AI, quantum computing, semiconductors — the sectors defining the next decade. The same depth institutional desks have, without the institutional price tag.",
  },
  {
    icon: "💬",
...
export default function Login({ onLogin, onBack }) {
  ...
  async function handleSignUp(e) {
    ...
    const { data, error } = await supabase.auth.signUp({
      email, password,
      options: { data: { full_name: name.trim() || null, plan } },
    });
    setLoading(false);
  ...
  const Tabs = () => (
    <div style={{
      display: "flex", borderBottom: "1px solid rgba(255,255,255,.07)", marginBottom: "2rem",
    }}>
      {[["signin", "Sign in"], ["plan", "Create account"]].map(([m, label]) => (
        <button key={m} onClick={() => switchMode(m)} style={{
          ...
          borderBottom: mode === m || (mode !== "signin" && m === "plan")
            ? "2px solid #00b4ff" : "2px solid transparent",
        ...
        {PLANS.map(p => {
          ...
          return (
            <button key={p.id} onClick={() => setPlan(p.id)} style={{
              background: selected ? "rgba(0,180,255,.06)" : "rgba(255,255,255,.02)",
              border: selected ? "1px solid rgba(0,180,255,.4)" : "1px solid rgba(255,255,255,.07)",
              borderRadius: 14, padding: "18px 18px", cursor: "pointer", textAlign: "left",
              boxShadow: selected ? "0 0 0 3px rgba(0,180,255,.08), 0 0 28px rgba(0,180,255,.12)" : "none",
              ...
              {p.recommended && (
                <span style={{
                  ...
                  color: "#fff", fontSize: ".62rem", fontWeight: 700, letterSpacing: ".14em",
                  textTransform: "uppercase", padding: "2px 10px", borderRadius: 999,
                  boxShadow: "0 2px 12px rgba(0,180,255,.35)",
                }}>Recommended</span>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                ...
                <div style={{ textAlign: "right", flexShrink: 0, paddingLeft: 12 }}>
                  <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, fontSize: "1.25rem", color: selected ? "#00b4ff" : "#6a8aac" }}>{p.price}</span>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 5 }}>
                {p.features.map((f, i) => (
                  <li key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: ".82rem", color: "#3d5c78" }}>
                    <span style={{ color: selected ? "#00b4ff" : "#2a4055", fontSize: ".65rem", paddingTop: 2, flexShrink: 0 }}>✓</span>
                    {f}
                  </li>
                ))}
              </ul>
    ...
    return (
      <div className="login-box">
        ...
        <div className="login-box-header">
          ...
          <p className="login-sub">You'll be charged after your free trial</p>
        </div>
  ...
  return (
    <div className="login-page">
      ...
      {/* left — story (always visible on desktop) */}
